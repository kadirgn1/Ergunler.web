(() => {
  ...
  const normTR = (value) =>
    (value ?? '').toString().toLocaleLowerCase('tr-TR').normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  ...
  const initCounters = () => {
    ...
    if (prefersReducedMotion) {
      counters.forEach((element) => {
        ...
        if (Number.isFinite(target)) element.textContent = `${target}${suffix}`;
      });
      return;
    }
  ...
  const applyCorporateContent = () => {
    const data = getLocalStorageObject('ergunler_corporate_content');
    applyTextMap({
  ...
  const applyFacilitiesContent = () => {
    const data = getLocalStorageObject('ergunler_facilities_content');
    applyTextMap({
      '#facility1TopText': 'facility1Top', '#facility1MainText': 'facility1Main', '#facility1Spec1Text': 'facility1Spec1', '#facility1Spec2Text': 'facility1Spec2', '#facility1Spec3Text': 'facility1Spec3',
      '#facility2TopText': 'facility2Top', '#facility2MainText': 'facility2Main', '#facility2Spec1Text': 'facility2Spec1', '#facility2Spec2Text': 'facility2Spec2', '#facility2Spec3Text': 'facility2Spec3',
      '#facility3TopText': 'facility3Top', '#facility3MainText': 'facility3Main', '#facility3Spec1Text': 'facility3Spec1', '#facility3Spec2Text': 'facility3Spec2', '#facility3Spec3Text': 'facility3Spec3'
    }, data);
  };

  const applyWhiteCalciteContent = () => {
    const data = getLocalStorageObject('ergunler_white_calcite_content');
    applyTextMap({
      '#whiteHeaderTitleText': 'whiteHeaderTitle',
      '#whiteHeaderLeadText': 'whiteHeaderLead',
      '#whiteSummaryTitleText': 'whiteSummaryTitle',
      '#whiteSummaryText': 'whiteSummaryText',
      '#whiteBadge1Text': 'whiteBadge1',
      '#whiteBadge2Text': 'whiteBadge2',
      '#whiteBadge3Text': 'whiteBadge3',
      '#whiteTechnicalTitleText': 'whiteTechnicalTitle',
      '#whiteTech1NameText': 'whiteTech1Name',
      '#whiteTech1ValueText': 'whiteTech1Value',
      '#whiteTech2NameText': 'whiteTech2Name',
      '#whiteTech2ValueText': 'whiteTech2Value',
      '#whiteTech3NameText': 'whiteTech3Name',
      '#whiteTech3ValueText': 'whiteTech3Value'
    }, data);
  };

  const applyFacilitiesAdvancedContent = () => {
    const data = getLocalStorageObject('ergunler_facilities_advanced_content');
    applyTextMap({
      ...
      // GRİ KALSİT SAYFASI İÇİN DÜZELTİLMİŞ ID EŞLEŞTİRMELERİ
      '#grayHeaderLeadText': 'facilityGrayKicker',
      '#graySummaryTitleText': 'facilityGrayTitle',
      '#graySummaryText': 'facilityGrayText',
      '#grayBadge1Text': 'facilityGrayBadge1',
      '#grayBadge2Text': 'facilityGrayBadge2',
      '#grayBadge3Text': 'facilityGrayBadge3',
  ...
  const initJobTableFilter = () => {
    ...
    const existingOptions = new Set(qsa('option', kurumSelect).map((option) => option.value));
    const uniqueKurums = Array.from(new Set(rows.map((row) => (row.dataset.kurum || '').trim()).filter(Boolean))).sort((a, b) => a.localeCompare(b, 'tr'));
    ...
    const applyFilter = () => {
      ...
      rows.forEach((row) => {
        ...
        const matchesKurum = selectedKurum === 'all' || (row.dataset.kurum || '') === selectedKurum;
  ...
  window.ErgunlerUI = {
    qs, qsa, normTR, prefersReducedMotion, safeJSONParse, getLocalStorageObject,
    applyTextMap, initPreloader, initNavbarScroll, initCounters, initMobileMenuClose,
    applyCorporateContent, applyFacilitiesContent, applyWhiteCalciteContent,
    applyFacilitiesAdvancedContent, initJobTableFilter
  };
})();